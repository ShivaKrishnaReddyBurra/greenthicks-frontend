...
export default function ServiceAreasView() {
  ...
  // Load Google Maps after service areas are fetched
  useEffect(() => {
    if (serviceAreas.length > 0 && !isMapLoaded) {
      // Check if API key exists first
      const apiKey = process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY
      if (!apiKey) {
        console.warn("Google Maps API key is not configured. Map will not be displayed.")
        return
      }
                ...
                {!process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY ? (
                  <div className="w-full h-96 lg:h-[600px] rounded-b-lg bg-gray-100 dark:bg-gray-800 flex items-center justify-center">
                    <div className="text-center p-8">
                      ...
                      <p className="text-gray-600 dark:text-gray-400">
                        Interactive map is currently unavailable. Please use the search function above to check service
                        areas.
                      </p>
                    </div>
                  </div>
