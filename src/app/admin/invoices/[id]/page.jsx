  ...
  useEffect(() => {
    const fetchInvoice = async () => {
      ...
      try {
        ...
        doc.text(`$${invoiceData.subtotal.toFixed(2)}`, margin + itemWidth + qtyWidth + priceWidth, y, { align: 'right' });
        y += 20;
        doc.text('Delivery Charge', margin + itemWidth + qtyWidth, y, { align: 'right' });
        doc.text(`$${invoiceData.shipping.toFixed(2)}`, margin + itemWidth + qtyWidth + priceWidth, y, { align: 'right' });
        if (invoiceData.discount > 0) {
          y += 20;
          doc.text('Discount', margin + itemWidth + qtyWidth, y, { align: 'right' });
          doc.text(`-$${invoiceData.discount.toFixed(2)}`, margin + itemWidth + qtyWidth + priceWidth, y, { align: 'right' });
        }
        y += 20;
        doc.text('Total', margin + itemWidth + qtyWidth, y, { align: 'right' });
        doc.text(`$${invoiceData.total.toFixed(2)}`, margin + itemWidth + qtyWidth + priceWidth, y, { align: 'right' });
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center">
            ...
            <h1 className="text-2xl font-bold text-gray-800 dark:text-white">Invoice #{params.id}</h1>
          </div>
          <Link
            href={`/admin/invoices/${params.id}?print=true`}
        ...
        {pdfUrl && (
          <iframe
            src={pdfUrl}
            className="w-full h-[80vh] border border-gray-200 dark:border-gray-700 rounded-xl"
            title="Invoice PDF"
          />
        )}
      </div>
